...
| 范围 | 实现与因果关系 | 验收含义 |
| --- | --- | --- |
| 节点快捷工具栏（EDIT-02） | `positionAndClampNodeBar()` 按顶部固定工具栏的实际下边界计算可用区域；选中节点离开可用画布时隐藏对应工具栏，窄画布中的操作条限制宽度并允许横向滚动。 | 节点工具栏不会被强行夹到顶部按钮后面；保留节点选择状态也不会让离屏节点的操作条悬在画布边缘。 |
...
该修复安装后，用户在原有导图中完成连续拖选并确认“很丝滑”。同次复测日志中，Markdown 的页面事件延迟平均 8.55 ms、P95 15 ms、最大 33 ms（108 次移动）；节点文字平均 10.48 ms、P95 32 ms、最大 48 ms（75 次移动）。它们是本次真实操作的观测结果，不作为所有设备的延迟上限。编译与原有 Swift 28 项检查也通过。
