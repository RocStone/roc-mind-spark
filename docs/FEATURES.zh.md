...
- **L** 在任意两个节点之间做交叉连接。
- **Space** 折叠 / 展开分支。节点边上的 **−** / **+** 是同一件事。全部折叠是一层一层收。
- **⌘F** 在当前图查找节点文字，被 **−** 收起的分支也搜得到。焦点留在搜索框，回车轮询下一项；画布定位到该节点，并展开到它的祖先链路。再回车时，若上一项的文字和结构没被改过，把这次为查看而临时展开的折叠收回。搜索框开着时再按 **⌘F** 关闭。**⌘H** 打开查找并替换。工具栏 🌐 可改成搜所有图。
- 地图编辑有撤销 / 重做。节点文字、Markdown、笔记的撤销跟当前焦点走，不会和地图历史混在一起。
- 节点内格式：粗体、斜体、下划线、删除线、字号、颜色、高亮、对齐。
- 标记、超链接、待办勾选、数学公式、节点计算公式（以 `=` 开头，支持运算及子节点汇总）、引用（你主动查 DOI 时才会请求 Crossref）、Markdown 表格、代码块、分隔线。节点里的 GFM 表格在未编辑时画成表，进入编辑后显示 Markdown 原文。
- 节点上可以挂图，再点一次打开查看。
